...
class MacosProcess implements ModuleProcess{
  ...
  void _checkResult(List<ProcessResult> result) {
    if(result.first.outText.contains('command not found')) {
      throw CommandNotFoundException('Yarn');
    }
  }
  ...
  Future<bool> install() async {
    var result = await shell.run('brew install yarn');
    _checkError(result);
    // todo 测试
    return true;
  }
  ...
  Future<bool> remove() async {
    var result = await shell.run('brew uninstall yarn');
    _checkError(result);
    // todo 测试
    return true;
  }
  ...
  Future<bool> update() async {
    var result = await shell.run('brew upgrade yarn');
    _checkError(result);
    // todo 测试
    return true;
  }
  ...
  Future<bool> checkInstall() async {
    var result = await shell.run('yarn --version');
    _checkResult(result);
    return true;
  }
