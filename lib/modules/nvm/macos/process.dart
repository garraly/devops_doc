...
class MacosProcess implements ModuleProcess{
  ...
  void _checkResult(List<ProcessResult> result) {
    if(result.first.outText.contains('command not found')) {
      throw CommandNotFoundException('Nvm');
    }
  }
  ...
  Future<bool> install() async {
    var result = await shell.run('brew install nvm');
    _checkError(result);
    // todo 测试
    return true;
  }
  ...
  Future<bool> remove() async {
    var result = await shell.run('brew uninstall nvm');
    _checkError(result);
    // todo 测试
    return true;
  }
  ...
  Future<bool> update() async {
    var result = await shell.run('brew upgrade nvm');
    _checkError(result);
    // todo 测试
    return true;
  }
  ...
  Future<bool> checkInstall() async {
    var result = await shell.run('nvm --version');
    _checkResult(result);
    return true;
  }
